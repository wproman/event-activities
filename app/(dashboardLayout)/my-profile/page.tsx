      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between mb-1">
            <h4 className="font-medium text-sm">{review.reviewer?.name || "Anonymous"}</h4>
...
const EventCard = ({ event, showHost = true }: { event: Event; showHost?: boolean }) => (
...
const ProfilePage: React.FC = async () => {
  ...
  if (Array.isArray(reviewsData)) {
    const hostReviews = reviewsData.filter((review: Review) => 
      review.recipientId === currentUser.id
    );
    reviewCount = hostReviews.length;
    totalRating = hostReviews.reduce((sum: number, item: Review) => sum + item.rating, 0);
  }
            ...
            <div className="flex flex-col md:flex-row md:items-end gap-4 -mt-12 md:-mt-16">
              <div className="relative">
                ...
                <div className="absolute bottom-0 right-0">
                  <Badge variant={
                    currentUser.status === 'ACTIVE' ? 'default' :
                    currentUser.status === 'BLOCKED' ? 'destructive' : 'secondary'
                  }>
                    {currentUser.status}
                  </Badge>
                </div>
              ...
              <div className="flex-1">
                <div className="flex flex-wrap items-center gap-3 mb-2">
                  ...
                  <Badge variant={
                    currentUser.role === "HOST" ? "secondary" : 
                ...
                <div className="mt-3 flex gap-4 text-sm">
                  <div>
                    <span className="font-semibold">{currentUser.ratingAvg.toFixed(1)}</span>
                    <span className="text-muted-foreground ml-1">avg rating</span>
                  </div>
                  <div>
                    <span className="font-semibold">{currentUser.ratingCount}</span>
